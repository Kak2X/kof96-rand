...
.cpu1P:
	...
	; Autoconfirm CPU opponent cursor in boss stages.
	; Having CHARSEL_MODE_CONFIRMED already will also be used later on to determine how to draw the character icons.
	ld   a, [wCharSeqId]				; A = New stage number
	cp   STAGESEQ_KAGURA				; In a boss/extra stage?
...
;	; If this is a boss round, keep whatever the win screen set us
;	ld   a, [wCharSeqId]				; A = New stage number
;	cp   STAGESEQ_KAGURA				; In a boss/extra stage?
;	jr   nc, .bossSeqSpecial			; If so, skip
...
;	ld   a, [wJoyActivePl]
;	or   a						; Playing on the Pl1 side? (== PL1, boss on 2P side)
;	jp   z, .bossSeqSpecial_do	; If so, jump
;	ld   bc, wCharSelP1CursorMode
;.bossSeqSpecial_do:
;	ld   a, CHARSEL_MODE_CONFIRMED
;	ld   [bc], a
;	; Reuse this code
...
.chkInitialMode:
	...
	; If all characters are already selected, switch to CHARSEL_MODE_READY.
	;
	; This is done by checking if the last character slot is empty.
	;
	
	
	; Check the last valid character slot.
	; This doesn't matter for CPU mode.
	ld   hl, wCharSelP1Char2
...
.singleChkIconP1:
	...
	; Determine ptr to char id
	; See .fillSelChars1P for more info.
	ld   hl, wCharSelP1Char0					; HL = Ptr to local Char ID
...
.singleChkIconP2:
	...
	; Determine ptr to char id
	; See .fillSelChars1P for more info.
	ld   hl, wCharSelP2Char0					; HL = Ptr to local Char ID
...
;	ld   a, [wCharSeqId]
;	cp   STAGESEQ_KAGURA		; Fighting a boss/extra next?
;	jp   nc, .team2PDraw		; If so, skip
.team1PDrawEmpty:
...
;	ld   a, [wCharSeqId]
;	cp   STAGESEQ_KAGURA		; Fighting a boss/extra next?
;	jp   nc, .fillSelChars1P	; If so, skip
.team2PDrawEmpty:
	; Draw second and third placeholder
...
.fillSelChars1P:
	...
	; Pick the ptr to the first team member ID.
	; If we got here from a round that autoconfirmed whatever we had, draw the icons
	; by the wPlInfo character IDs and not the copy local to the character select screen.
	; We don't even have the latter properly set in that case, so it'd draw garbage.
	
	ld   hl, wCharSelP1Char0					; HL = Ptr to local Char ID
	ld   a, [wCharSelP1CursorMode]
	cp   a, CHARSEL_MODE_CONFIRMED				; Already confirmed?
	jr   nz, .fillSelChars1P_do					; If not, skip
	ld   hl, wPlInfo_Pl1+iPlInfo_TeamCharId0	; Otherwise, use the confirmed one
...
.pl1:
	...
	jr   nz, .randomPick		; If so, jump
	ld   a, [wCharSelRouletteFlags1P]
	ld   hl, wCharSelP1Char0
	jr   .chkRoulette
.pl2:
...
; IN
; - A: Roulette flags
; - HL: Ptr to first team member
.chkRoulette:

	ld   c, a	; Move this here
	
	; Seek to the current slot and check if it has the roulette flag set.
	; If so, autopick it
	ld   a, CHAR_ID_NONE
	cp   a, [hl]				; Is the first slot empty?
	jr   z, .chkRoulette_slot1	; If so, check if it should be randomized
	inc  hl
	cp   a, [hl]				; Is the second slot empty?
	jr   z, .chkRoulette_slot2	; If so, check if it should be randomized
	; Otherwise, assume the third slot needs checking
.chkRoulette_slot3:
	bit  CRFB_CHAR2, c		; Is this a roulette slot?
	jr   z, .doCtrl			; If not, skip
	jr   .randomPick
.chkRoulette_slot2:
	bit  CRFB_CHAR1, c		; Is this a roulette slot?
	jr   z, .doCtrl			; If not, skip
	jr   .randomPick
.chkRoulette_slot1:
	bit  CRFB_CHAR0, c		; Is this a roulette slot?
	jr   z, .doCtrl			; If not, skip
	jr   .randomPick
...
.doCtrl:
	...
	; SELECT -> Removes all selected characters and clears roulette status.
...
.pl1:
	ld   hl, wCharSelP1Char2
	ld   de, wCharSelRouletteFlags1P
	call CharSel_ClearSlotV2	; Try to clear a slot
	jr   c, .ret				; If nothing found, return
	call CharSel_ClearTopIcon1P	; Otherwise, also remove the char icon
	jp   .ret
.pl2:
	ld   hl, wCharSelP2Char2
	ld   de, wCharSelRouletteFlags2P
	call CharSel_ClearSlotV2
	jr   c, .ret
...
; IN
; - HL: Ptr to third selected character
; - DE: Ptr to to roulette flags
; OUT
...
CharSel_ClearSlotV2:
	...
	;
	call IsInTeamMode		; Are we in team mode?
	jr   c, .team			; If so, jump
.single:
	ld   b, $00				; B = No characters selected
	
	; Seek back to the first character slot
	dec  hl					; HL -= 2
	dec  hl
	
	ld   a, CHAR_ID_NONE	; A = Comparison value
	ld   c, $FF^CRF_CHAR0	; C = Initial mask
	jp   .chkSlot0
...
.clearSlot:
	...
	ld   a, [de]			; Clear roulette flag for the slot
	and  c
	ld   [de], a
...
.pl1:
	...
	cp   CHARSEL_ID_ROULETTE
	jr   z, .enableRoulette1P
	ld   c, a
...
.enableRoulette1P:
	ld   de, wCharSelRouletteFlags1P	; DE = Ptr to roulette Flags
	ld   hl, wCharSelP1Char0			; HL = Ptr to first team member
	jr   .enableRoulette
.enableRoulette2P:
	ld   de, wCharSelRouletteFlags2P
	ld   hl, wCharSelP2Char0
.enableRoulette:
	...
	; Set the flags for enabling the roulette for one or more characters.
	; From the point roulette is chosen, the remaining characters are also roulette'd.
	ld   b, a
		ld   c, CHAR_ID_NONE	; C = Comparison value
		ldi  a, [hl]			; Read team member
		cp   c					; Is it empty?
		jr   z, .slot0			; If so, jump
		ldi  a, [hl]			; Read team member
		cp   c					; Is it empty?
		jr   z, .slot1			; If so, jump
		jr   .slot2
		; Set the flags
	.slot0:
		set  CRFB_CHAR0, b
...
CharSel_IdTilesMapTbl:
	...
	dw $8B60 ; CHARSEL_ID_ROULETTE (impossible to see)
...
.chkLockRoulette:
	...
	ld   a, [wMisc_C025]
	bit  MISCB_IS_SGB, a		; Playing in SGB mode? (no Serial VS)
	jr   nz, .charOk			; If so, skip
	
	; Otherwise, clear the roulette slot.
	; This will also blank the slot during endless CPU vs CPU mode, which is fine.
	ld   a, CHAR_ID_NONE
	ld   [wCharSelIdMapTbl+CHARSEL_ID_ROULETTE], a
	jr   .nextChar
