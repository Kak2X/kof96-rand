...
; =============== Win_ChkResetChars ===============
; Clears out team characters, if needed.
Win_ChkResetChars:
	...
	jr   z, .chkRoulette					; If not, return
...
.chkRoulette:
	...
	; Delete all characters flagged roulette
	; This will never happen for the CPU side.
	
	; This assumes that the only characters flagged as "roulette" are at the end of the list.
	; This is the behaviour of other KOF games with roulette, so we're going with it.
...
; IN
; - A: Roulette flags
; - HL: Ptr to third team member
.doRouChk:
	ld   c, a			; Move them out to C
	
	ld   a, [wPlayMode]
	bit  MODEB_TEAM, a	; In TEAM Mode?
	jr   nz, .chkTeam2	; If so, jump
.single:
	dec  hl				; Seek to 1st member
	;dec  hl
	jr   .chkTeam0
	
.chkTeam2:
	bit  CRFB_CHAR2, c	; Is the 3rd character a roulette one?
	jr   z, .chkTeam1	; If not, skip
	ld   [hl], CHAR_ID_NONE
.chkTeam1:
	dec  hl
	
	bit  CRFB_CHAR1, c	; Is the 2nd character a roulette one?
	jr   z, .chkTeam0	; If not, skip
	ld   [hl], CHAR_ID_NONE	
.chkTeam0:
	dec  hl
	
	bit  CRFB_CHAR0, c	; Is the 1st character a roulette one?
	ret  z				; If *NOT*, return
	ld   [hl], CHAR_ID_NONE
	ret
